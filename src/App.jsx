...
const Dashboard = ({ onStartScan, onStartIDScan, documents, onShare, onDelete, scanningError, isDarkMode, onToggleTheme }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen p-6">
      ...
      <div className="glass mb-8 px-4 py-3.5 flex items-center gap-3 bg-input border-white/5 shadow-inner shrink-0">
      ...
      <div className="flex-1 space-y-4 mb-10 overflow-y-auto">
        {filteredDocs.length === 0 ? (
          <div className="glass p-16 flex flex-col items-center justify-center border-dashed border-2 border-white/5 bg-white/[0.01]">
            ...
            <p className="text-muted text-xs font-medium text-center">Buzón de escaneos vacío</p>
          </div>
        ) : (
          filteredDocs.map((doc, i) => (
            <motion.div 
              key={doc.name} 
              className="glass p-4 flex items-center gap-4 bg-card border-white/5"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[9px] text-primary mt-1 uppercase font-bold tracking-widest opacity-60">PDF Document</p>
              </div>
              <div className="flex gap-1 shrink-0">
                <button 
                   onClick={() => onShare(doc.uri, doc.name)} 
                   className="p-3 text-primary opacity-60 active:scale-90 transition-transform bg-input rounded-xl"
                >
                    <Share2 size={16} />
                </button>
                <button 
                   onClick={() => onDelete(doc.name)} 
                   className="p-3 text-accent opacity-60 active:scale-90 transition-transform bg-input rounded-xl"
                >
                    <Trash2 size={16} />
                </button>
              </div>
